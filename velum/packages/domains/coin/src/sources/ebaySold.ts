 ...
 *       ?q=<label>&category_ids=11116[&limit=<limit>]
 *   (11116 = catégorie eBay « Coins & Paper Money »)
...
const EBAY_COINS_CATEGORY = '11116';
...
export class EbaySoldSource implements PriceSource {
  ...
  async fetch(query: PriceQuery): Promise<PriceObservation[]> {
    try {
      const params: Record<string, string> = { q: query.label, category_ids: EBAY_COINS_CATEGORY };
