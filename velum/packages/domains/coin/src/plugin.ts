...
export class CoinDomainPlugin implements DomainPlugin<CoinAnalysisPayload> {
  ...
  async recognize(input: CaptureInput, deps: RecognizeDeps): Promise<RecognitionResult> {
    ...
    // down) doit remonter en 429/503. La déguiser en « saisie assistée » rend la
    // panne invisible. `UNREADABLE_RECOGNITION` ne signifie plus qu'une chose :
    // le modèle a répondu, mais n'a rien su identifier.
    let raw: string;
    if (input.kind === 'photo') {
